...
const transformItems = (itemsObj) => {
  ...
  const obj={data_url:`${process.env.REACT_APP_IMG_SERVER}/images/${itemsObj.profile}`}
  const obj2={data_url:`${process.env.REACT_APP_IMG_SERVER}/images/${itemsObj.cover}`}
  image.push({...obj})
...
if(account!==undefined && account!=="" && account!==null){
  ...
  fetchUser(
	{ url: `${process.env.REACT_APP_IMG_SERVER}/createUser`,method:'POST',headers: { 'Content-Type': 'application/json; charset=utf-8',"Access-Control-Allow-Origin":"*","Access-Control-Allow-Headers": "Origin, X-Requested-With, Content-Type, Accept" }, body: {address:account} },
	transformItems
  );
}
},[account]);
...
const res= await fetch(`${process.env.REACT_APP_IMG_SERVER}/api/user/img`,data,{
	method:'PUT',
	headers:{
...
const modifyUser=(event)=>{
	...
	fetchUser(
		{ url: `${process.env.REACT_APP_IMG_SERVER}/modifyUser`,method:'PATCH',headers: { 'Content-Type': 'application/json; charset=utf-8' }, body: {address:account,bio:bio,name:name,twitter:twitter,instagram:instagram,website:website,email:email} },
		transformItems
	  );
}
				...
				<div className="row row--grid">
					<div className="col-12 col-xl-12">
						<div className="main__title main__title--page mb-4">
							<h1>Edit profile</h1>
							<p className="mt-0 json-gray">You can set preferred display name, create your branded profile URL and manage other personal settings</p>
						</div>
					</div>
								...
								<div className="author__meta">
									<div className="row">
										...
										<div className="col-sm-12 col-md-12 col-lg-12 mt-3">
											<button className="btn-primary" onClick={modifyUserImage}>Update</button>
										</div>
									</div>
								</div>
							</div>
								...
								<div className="author__meta">
									<form className="sign__form sign__form--profile mt-0">
										<div className="sign__group">
											...
											<input id="username" type="json" name="username" className="sign__input" placeholder="Enter your display name" value={name} onChange={(event)=>setName(event.target.value)}/>
										</div>
										<div className='sign__group'>
											<label className="sign__label required-field" htmlFor="short-bio">Short Bio</label>
											<jsonarea id="short-bio" name="short-bio" className="sign__jsonarea" placeholder="Tell about yourself in a few words" value={bio} onChange={(event)=>setBio(event.target.value)}></jsonarea>
										</div>
										<div className="sign__group">
											<label className="sign__label" htmlFor="email">Email Address</label>
											<input id="email" type="json" name="email" className="sign__input" placeholder="Enter your email" value={email} onChange={(event)=>setEmail(event.target.value)} />
										</div>
										...
										<div className="sign__group">
											...
											<input id="twitter" type="json" name="twitter" className="sign__input" placeholder="Enter your Twitter Link" value={twitter} onChange={(event)=>setTwitter(event.target.value)}/>
										</div>
										<div className="sign__group">
											<label className="sign__label" htmlFor="instagram">Instagram Link</label>
											<input id="instagram" type="json" name="instagram" className="sign__input" placeholder="Enter your Instagram Link" value={instagram} onChange={(event)=>setInstagram(event.target.value)}/>
										</div>
										<div className="sign__group">
											<label className="sign__label" htmlFor="sitelink">Personal site or portfolio</label>
											<input id="sitelink" type="json" name="sitelink" className="sign__input" placeholder="https://yoursitelink.com" value={website} onChange={(event)=>setWebsite(event.target.value)}/>
										</div>
										<button className="btn-primary" onClick={modifyUser}>Submit</button>
									</form>
								</div>
							</div>
